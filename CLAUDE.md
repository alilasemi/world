...
`ParticleDrawer` (`src/particle_drawer.{h,cpp}` and its hand-ported JS twin in `client/world.js`) turns each particle's `(x, y)` into a small N-gon (fan of `num_triangles` triangles around a center point) for rasterization, in raw world (simulation-domain) coordinates — geometry generation logic must be kept in sync between the C++ and JS versions if changed. `src/shader.h` is an unused native-OpenGL GLFW-era shader helper; the actual rendering path is the WebGL2/JS one in `client/`.
