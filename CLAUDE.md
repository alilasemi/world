...
1. `FindNeighborsKernel` (src/find_neighbors_kernel.{h,cu}) — buckets particles into a uniform `grid_size x grid_size` conceptual spatial grid, backed by two `cuco::static_map<int,int>` hash maps it owns privately (see "Spatial grid" below), then gathers each particle's neighbors (via the same maps) into a flat `device_neighbors` array for `ComputeRHSKernel` to consume; `particles_per_cell` capacity per cell, hard assert-fails on overflow.
...
3. `ComputeRHSKernel` (src/compute_rhs_kernel.{h,cu}) — computes forces per particle: gravity, a penalty-based floor/wall repulsion, a penalty + damping repulsion against neighbors read from `device_neighbors` (no grid/cuco knowledge of its own — see "Spatial grid" below), and (added last) the interpolated `body_force_x/y` from step 2.
...
### Spatial grid: `GridMap` hash maps, isolated entirely inside `FindNeighborsKernel`

The 256×256 collision grid used to be one dense `DeviceVector<int>` of size `grid_size*grid_size*particles_per_cell` — O(m²·k) memory regardless of how many particles actually exist. It's now two `cuco::static_map<int,int>` hash maps (`src/grid_map.h`), so memory scales with particle count `n` instead of grid resolution: `num_particles_in_cell` maps `cell_index` (`cell_x*grid_size+cell_y`) -> particle count for that cell; `particles_in_cell` maps the composite key `cell_index*particles_per_cell+slot` -> particle index. Capacity for both is sized off `n` (`2*min(n, grid_size*grid_size)` and `2*n` respectively) — capacity bounds the *number of entries* a hash map holds, not the numeric range of its keys, so `particles_in_cell`'s composite keys can range far above its capacity with no issue.

**`FindNeighborsKernel` owns both maps privately** (constructed in its own constructor, destroyed in its own destructor — both defined in `find_neighbors_kernel.cu`) and is the *only* file in this codebase that touches cuco at all. `ParticleDynamicsCUDA` and `ComputeRHSKernel` never see a `GridMap`; instead, `FindNeighborsKernel` writes its results into a plain `DeviceVector<int> device_neighbors` (owned by `ParticleDynamicsCUDA`, sized `n*9*particles_per_cell` — 9 = the 3x3 stencil's cell count) that `ComputeRHSKernel` just reads. This interface choice (a flat array instead of passing `GridMap`/cuco-ref handles between kernels) is deliberate: a `__global__` kernel parameter passed by value (like a cuco ref) needs its *complete* type visible wherever the kernel is both defined and launched — there's no clean way to forward-declare-and-link-later for that, the way `grid_map_fwd.h` lets host code hold an opaque `GridMap*`. A flat `int*` has no such constraint, so it's what lets `particle_dynamics_cuda.cu`/`compute_rhs_kernel.cu` stay completely free of cuco's ~10s-per-file template-instantiation compile cost (verified by timing `cudafe++`/`cicc`/host-`gcc` separately — it's instantiation cost, not parsing, so isolating cuco to fewer files is the only real lever; a prior attempt to also move the `__global__` kernel *bodies* themselves out of their own `.cu` files into a shared one was reverted — kernels stay textually in their own files, only the cuco-touching plumbing around them moved).

`FindNeighborsKernel::call_kernel` does two launches per step. First, `clear()` on both maps (host-side, cheap now that capacity is O(n) not O(m²)) then one `populate_grid_kernel` launch buckets particles: per particle, `num_particles_in_cell_ref.insert_and_find({cell_index, 0})` gets-or-creates the count entry and returns a slot reference, then `cuda::atomic_ref<int, cuda::thread_scope_device>{...}.fetch_add(1, ...)` atomically reserves this particle's slot number; `particles_in_cell_ref.insert({cell_index*particles_per_cell+slot, i})` then records the particle index at that slot. Second, a `find_neighbors_kernel` launch re-walks the same 3x3 stencil via `find()` on both (now fully populated) maps and writes each particle's found neighbors (excluding itself) into its row of `device_neighbors`, terminated by a `-1` sentinel — a cell that received no particles this step was simply never inserted, so `find()` returning "not found" is a well-defined "0 particles," not an uninitialized-memory risk. `ComputeRHSKernel`'s body just reads `neighbors[i*(9*particles_per_cell) ...]` sequentially until hitting `-1`. Verify any change here with `compute-sanitizer --tool racecheck` (the atomic-increment-via-map idiom in the bucketing pass) and `--tool initcheck` (both the map lookups and the `device_neighbors` read/write).
...
`<cuco/static_map.cuh>` cannot be parsed by a plain host compiler at all (`cuco/detail/__config` explicitly errors out without `__CUDACC_VER_MAJOR__`) — but `ParticleDynamicsCUDA` is declared in a header that plain g++-compiled `.cpp` files (`profiling_sim.cpp`, `broadcast.cpp`, the tests) also include, per the existing "host code that just calls into the class" pattern (see Testing below). `src/grid_map_fwd.h` (just `class GridMap;`, safe for any `.cpp`/`.h`) is what `find_neighbors_kernel.h` includes for its private `std::unique_ptr<GridMap>` members — the only header that still needs it, now that `particle_dynamics_cuda.h`/`compute_rhs_kernel.h` don't reference `GridMap` at all. `src/grid_map.h` (the real `cuco::static_map`-based definition) is only included from `find_neighbors_kernel.cu`. `GridMap` has no default constructor (capacity/sentinels are required at construction) and its destructor needs the complete type, which is why `FindNeighborsKernel` declares (but doesn't `= default` inline) its own destructor in the header, defining it out-of-line in the `.cu` where `grid_map.h` is visible — the standard rule for a `unique_ptr<Incomplete>` member, here applied to the *kernel class* rather than `ParticleDynamicsCUDA` itself.
