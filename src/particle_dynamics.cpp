...
//        force_y += c_r[mat][0] * powf(1 / (y - floor_y), 4); // Repulsive force
        force_y += fmax(0, fmin(max_force, max_force / (r0 - r1) * (y - floor_y - r1)));
        force_y -= c_d[mat][0] * powf(1 / (y - floor_y), 2) * vy; // Damping based on velocity
//        // Wall on the left (at -1)
//        force_x += c_r[mat][0] * powf(1 / (x - wall_x), 4); // Repulsive force
//        force_x -= c_a[mat][0] * powf(1 / (x - wall_x), 2); // Attractive force
//        force_x -= c_d[mat][0] * powf(1 / (x - wall_x), 2) * vx; // Damping based on velocity
        // Particle collisions
...
//                // Repulsive force
//                force_x += c_r[mat][mat_j] * powf(1 / dist, 8) * (dx / dist);
//                force_y += c_r[mat][mat_j] * powf(1 / dist, 8) * (dy / dist);
//                // Damping based on relative velocity
                force_x -= c_d[mat][mat_j] * powf(1 / dist, 2) * (vx - vx_j);
                force_y -= c_d[mat][mat_j] * powf(1 / dist, 2) * (vy - vy_j);
//                // Attractive force
//                force_x -= c_a[mat][mat_j] * powf(1 / dist, 2) * (dx / dist);
//                force_y -= c_a[mat][mat_j] * powf(1 / dist, 2) * (dy / dist);
float force = fmax(0, fmin(max_force, max_force / (r0 - r1) * (dist - r1)));
