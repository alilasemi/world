...
__global__ void occupancy_grid_kernel(int* occupancy, const float* state, size_t n, int m,
    ...
    for (int i = index; i < n; i += stride) {
        const int cell_x = min(m - 1, max(0, static_cast<int>(
                (state[4 * i + 0] - domain.x_min) / (domain.x_max - domain.x_min) * m)));
        const int cell_y = min(m - 1, max(0, static_cast<int>(
                (state[4 * i + 1] - domain.y_min) / (domain.y_max - domain.y_min) * m)));
...
void OccupancyGridKernel::call_kernel(int blocks, int threads_per_block) {
    ...
    occupancy_grid_kernel<<<blocks, threads_per_block>>>(occupancy, state, n, m, domain);
}
