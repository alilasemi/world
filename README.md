...
accept. Both halves are here: the exact dynamics that a learned model is usually asked to
stand in for, and the learned model, scored against the exact answer on held-out trajectories.
...
since contacts open and close and the tangential force saturates at the Coulomb limit, and they
...
after it, and including it is what makes the total monotonically decreasing.

The simulation serves state over a WebSocket, and the browser client renders the positions it
receives directly, which is why nothing sits between a state and a picture. Every physical and
numerical quantity is read from a configuration file, so a counterfactual of the form "the same
scene at restitution 0.3" is an edit to a text file.
...
by jamming: growth stops when the deposit settles and friction locks the packing, which is why
the plateau sits at 1.9 grain diameters instead of at the scale of the assembly. The horizon is
therefore set by how long the material flows, and the textbook estimate
`(1/lambda) ln(D_sat/eps)` gives 1.38 s against the measured 0.62 s because the early growth
far outruns the sustained rate.
...
is not a Markov state. The deposit is continuous in particle position, which nearest-node
binning is not, and the discontinuity that binning would introduce at every cell crossing would
...
That field is reduced by a proper orthogonal decomposition (POD), computed by singular value
decomposition (SVD) of the centered snapshot matrix, with one basis per channel. A joint basis
would require the four channels to be scaled against one another, and that scaling is not merely
awkward: mass density is positive definite while momentum density is signed, and the dynamic
range of momentum is both wider and more scenario-dependent, so whichever channel were scaled
larger would dominate the spectrum and the reported energy captured would stop describing
anything.
...
The right panel settles a question that a single number would have answered wrongly. Asked at
...
modes of 0.35 to 0.40 at t = 0.2 s against 0.34 for mass. The same measurement settles the
Markov argument quantitatively: at 0.2 to 0.4 s momentum is 2.3 times the mass scale, so a
density-only state would omit the dominant part of the state.
...
trajectory into training. The action parameters are withheld deliberately: the launch velocity
has finished acting by the first recorded snapshot and is carried in the momentum channels, so
supplying it again would let the model bypass the reduced state and make the sufficiency of that
state untestable.
...
realization about the ensemble mean, and taking the pairwise difference between two realizations
instead would have inflated it by a factor of sqrt(2) and made the model look nearly optimal
when it is not. The value is specific to a configuration and a horizon, so
`surrogate/fit_dynamics.py` takes it as an argument rather than hardcoding one.

Decomposing the error into three separately measurable terms is what made the difference,
...
error is the teacher-forced error minus that floor; compounding is the rollout error minus the
teacher-forced error. At a latent rank of 16 the three terms were 0.401, 0.000 and 0.063, so
truncation carried 86 percent of the error and additional training data would have done nothing
at all: the teacher-forced error equalled the truncation floor to four decimal places. Raising
the rank to 96 for mass and 24 per momentum axis inverted the budget, after which doubling the
prediction interval halved the number of compounding events and narrowing each parameter range
by about 40 percent improved both remaining terms at once, since a fixed training subset covers
a smaller domain more densely.
...
Two caveats belong with the number 0.174. The accuracy was bought with generality: the parameter
box is about 40 percent narrower per axis and therefore some 7 percent of the original volume in
seven dimensions, so the figure must never be quoted without its domain. And exact GP inference
is cubic in the number of training points, so anything past roughly 1500 is prohibitive; what a
larger database buys is a better basis and a better-covering subset, which is why narrowing the
parameter box was the effective lever and the row count was not.
...
and the remainder renormalized, because a truncated reduction rings and leaves a diffuse halo of
...
true value of 3.62. Enforcing minimum separation costs accuracy in the low-order moments, since
the sampler cannot exceed the local packing limit, so uniform placement stays preferable for
visualization and bulk statistics.
...
The world model is not yet the faster of the two, and the reason is worth stating. One
...
Unit tests. Eleven tests construct the simulation directly and cover the deposit properties of
...
Reverting either half of the over-capacity fix makes that test read past the end of a row, so it
covers the regression instead of merely passing.
...
The front radius against laboratory experiment. Lube et al. [[12]](#ref12) correlated the final
...
Stability and accuracy. A sweep of the timestep against the contact stiffness, each combination
...
roughly 5 of which nothing works, and the training configurations therefore run ten times finer
than the interactive demonstration. Before that was fixed, 5 of 8 rollouts diverged, every one at
the first checkpoint and with no grain having escaped the domain, and it is that second fact which
identified numerical blow-up as the cause.
...
1. There is no perception. Grains are rendered as untextured shaded spheres, and the appearance
   gap to a camera is total.
...
3. The rollout is accurate over a parameter box some 7 percent of the original volume, and the
   remaining error is dominated by compounding, which would need an objective defined over a
   multi-step rollout that the exact GP formulation cannot express.
...
5. The code is not differentiable, and the omission is deliberate. The pathwise gradient of a
   ...
   zeroth-order estimation over parallel rollouts [[24]](#ref24) takes the gradient's place.
   Automatic differentiation would win for high-dimensional structured parameters, such as a
   spatially varying friction field or a learned constitutive model, and that is the honest
   boundary of the argument.
6. The timestep is limited by the contact stiffness. Scaling the stiffness or the particle count
   drives it down and the real-time budget collapses, which is the wall between the present grain
   counts and one million.
7. One GPU, with no domain decomposition across devices.
8. When an impact is energetic enough to reach the force clamp, the achieved restitution falls
...
9. There is no rolling resistance, so heaps settle at about 15 degrees where dry sand sits near
   ...
   static threshold.
10. Sub-millimetre-per-second dynamics near the domain walls are unresolved, for the float32
    reason given in Section [7](#7-verification-and-validation). Double-precision positions would
    fix it and are not implemented.
